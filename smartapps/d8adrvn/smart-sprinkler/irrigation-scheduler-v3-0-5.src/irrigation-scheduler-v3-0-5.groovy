...
preferences {
	...
	page(name: "weatherPage", title: "Virtual Weather Station Setup", install: true) {
        ...
        section("Zip code to check weather...") {
            input "zipcode", "text", title: "Enter zipcode", required: true
        }
        
        section("Weather Station ID to check weather...") {
            input "pwsId", "text", title: "Enter your station ID", required: true
        }
        
        section("Weather Undergrond API key...") {
            input "wuApiKey", "text", title: "Enter your API key", required: true
        }
...
def scheduleCheck() {
    ...
    sendPush("Watering day ${state.daysSinceLastWatering[state.currentTimerIx] ?: 0} of ${days}")
    
// 	  Next line is useful log statement for debugging why the smart app may not be triggering.
//    log.info("${app.label} scheduler state: ${schedulerState}. Days since last watering: ${daysSince()}. Is watering day? ${isWateringDay()}. Enough time? ${enoughTimeElapsed(schedulerState)} ")
...
def isWeatherDelay() { 
 ...
 //     check to see if virtual rainguage exceeds threshold
        ...
        if (maxThermometer && maxThermometer < (tempThreshold?.toFloat() ?: 0)) {
        	if (isNotificationEnabled.equals("true")) {
...
{
    def params = [
        uri:  'https://api.weather.com/v2/pws/',
        path: 'dailysummary/7day',
        contentType: 'application/json',
        query: [stationId:"${pwsId}", format:'json', units:'e', apiKey:"${wuApiKey}"]
    ]
    def yesterdaysInches = 0
    try {
        httpGet(params) {resp ->
            //log.debug "resp data: ${resp.data}"
            yesterdaysInches = resp.data.summaries[5].imperial.precipTotal
    		log.info("Yesterday's percipitation for ${pwsId}: ${yesterdaysInches} in")
        }
    } catch (e) {
        log.error "error: $e"
    }    
...
def isWet() 
{
    ...
    try {
        httpGet(params) {resp ->
            //log.debug "resp data: ${resp.data}"
            todaysInches = resp.data.observations[0].imperial.precipTotal        
            log.info("Today's percipitation for ${pwsId}: ${todaysInches} in")
        }
    } catch (e) {
        log.error "error: $e"
    }
...
{
    def params = [
        uri:  'https://api.weather.com/v3/wx/',
        path: 'forecast/daily/5day',
        contentType: 'application/json',
        query: [postalKey:"${zipcode}:US", format:'json', units:'e', language:'en-US', apiKey:"${wuApiKey}"]
    ]
    def forecastInches = 0
    try {
        httpGet(params) {resp -> L:{
            //log.debug "resp data: ${resp.data}"
            	forecastInches = resp.data.qpf[0]
            	log.info("Forecast rain for ${zipcode}: ${forecastInches} in")
            }
        }
    } catch (e) {
        log.error "error: $e"
...
def isHot() 
{
   	def params = [
        ...
        path: 'forecast/daily/5day',
        contentType: 'application/json',
        query: [postalKey:"${zipcode}:US", units:'e', language:'en-US', format:'json', apiKey:"${wuApiKey}"]
    ]
    def todaysHighTemp = 50
    try {
        httpGet(params) {resp -> L:{
            //log.debug "resp data: ${resp.data}"
            	todaysHighTemp = resp.data.temperatureMax[0]
